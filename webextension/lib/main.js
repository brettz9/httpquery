...
const
  // eslint-disable-next-line no-shadow -- A global now?
...
const {observerBuilder} = require('./ObserverBuilder.js');

const {Cc} = chrome,
  {Ci} = chrome,
  {ACCESS_READ} = Ci.nsICache,
  ...
  checkQueryServerSupport = function (header, queryType) {
    const queryServerSupport = (/^accept-ranges:\s*(.*?)\s*(?:;|$)/im).exec(header);
    return queryServerSupport && (
      !queryType || (queryServerSupport[1].split(/\s*,\s*/).includes(queryType))
    );
  },
  /**
    * @param url
    * @param supportedCb
    * @param notSupportedCb
    * @param noEntryCb
    * @param {string} [queryType]
    */
  checkCachePerSupport = function (url, supportedCb, notSupportedCb, noEntryCb, queryType) {
    const checkCacheListener = {
      onCacheEntryAvailable (entry, access, status) {
        if (!entry) { // Called if not available
          return noEntryCb && noEntryCb(queryType);
        }
        const queryServerSupport = checkQueryServerSupport(entry.getMetaDataElement('response-head'), queryType);
...
exports.main = function () {
  ...
  httpQueryPanel.port.on('query', function (params) {
    const url = removeAnchor(params[0]),
      query = params[1],
      queryType = params[2],
      userFormat = params[3],
      ...
      setWindowContents = function (markup) {
        ...
        tabs.open({
          ...
          onReady (tab) {
            const oldDoc = getCurrentTabContentDocument(),
              newDoc = textToDocument(markup);
            oldDoc.replaceChild(newDoc.documentElement, oldDoc.documentElement);
          }
        });
      ...
      docEvaluateArray = function (expr, doc, context, resolver = null) {
        let i, result, a = [];
      ...
      serializeDOMNodes = function (arr, contentType, format) {
        const serializer = Cc['@mozilla.org/xmlextras/xmlserializer;1'].createInstance(Ci.nsIDOMSerializer),
          isXML = ['application/xml', 'application/xhtml+xml', 'application/tei+xml'].includes(contentType),
          mapFunc = (isXML || format === 'xml')
            ? function (resultItem) {
      ...
      executeXPath1OnDOMDocument = function (doc, contentType, format) {
        let markup = serializeDOMNodes(docEvaluateArray(query, doc), contentType, format);
        markup = format === 'json' ? JSON.stringify(markup) : markup.join('');
        handleMarkup(markup);
      },
      executeCSS3OnDOMDocument = function (doc, contentType, format) {
        let extended = query.match(/(.*?):(text|attr)\(([^)]*)\)$/),
          markup = extended
            ? (extended[2] === 'attr'
      ...
      queryNotSupportedDocument = function (entry) {
        let cacheData, doc, ctype, qctype, contentType, format,
        ...
        qctype = (/^query-content-type:\s*(.*?)\s*(?:;|$)/im).exec(
          entry.getMetaDataElement('response-head')
        );
        ctype = (/^content-type:\s*(.*?)\s*(?:;|$)/im).exec(
          entry.getMetaDataElement('response-head')
        );
      ...
      makeNewRequest = function (queryType) {
        const requestObj = {
          ...
          onComplete (resp) {
            let markup, doc, qctype, ctype, format, contentType,
  ...
  function changeIconPerResponseQuerySupport (subject, topic, data) {
    if (getCurrentTabContentWindow() === getWindowForRequest(subject)) { // If this is (or is equivalent to) the current tab, check whether to change icon
      let qss, channel = subject.QueryInterface(Ci.nsIHttpChannel);
