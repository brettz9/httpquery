...
function setStaticURL (res, url) {
  if (!(/\w/).test($h.url[0]) || (/\.\./).test($h.url)) {
    return exitError(res, 'Disallowed character in file name');
  }
  let pth = path.normalize(require('url').parse(url).pathname);
...
return {
  ...
  getExtensionForPath (path) { // or for Node, require('path').extname(path) but this includes "."
